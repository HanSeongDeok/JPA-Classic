...
class OrderServiceTest {
    ...
    public void order() throws Exception {
        ...
        System.out.println("주문한 상품 종류 수가 정확해야함");
        assertEquals(1, getOrder.getOrderItems().size());

        System.out.println("주문 가격은 가격 * 수량이다");
        assertEquals(10000 * orderCount, getOrder.getTotalPrice());

        System.out.println("주문 수량 만큼 재고가 줄어야 한다");
    ...
    public void cancelOrder() throws Exception {
        ...
        //then
        Order getOrder = orderRepository.findOne(orderId);
        System.out.println("주문 취소시 상태는 CANCEL 이다.");
        assertEquals(getOrder.getStatus(), OrderStatus.CANCEL);

        System.out.println("주문 취소된 상품은 그만큼 재고가 증가해야한다.");
        assertEquals(10, item.getStockQuantity());
    }

    @Test
    public void overStackedOrder() throws Exception {
        //given
        Member member = createMember();
        ...
        //then
        //assertEquals(item.getStockQuantity(), 9);

        assertThrows(NotEnoughStockException.class, ()->{
            System.out.println("재고 수량 예외 처리 확인");
            orderService.order(member.getId(), item.getId(), orderCount);
        });
    }
